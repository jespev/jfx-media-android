...
/*
 * $Id: XMLStreamFilterImpl.java,v 1.9 2010-11-02 21:02:27 joehw Exp $
 */
 ...
 *
 * @author Joe Wang: 
 * This is an effort to simplify the impl and make the filtered stream reader more compatible 
 * with those in other implementations. Note however, that this version will not solve all the issues
 * related to the undefined condition in the spec. The No.1 priority is that the simplified impl would
 * still pass the TCK. Problems arising due to the requirement, such as if hasNext() should advance 
 * the underlining stream, would have to wait until 1.1 in which the filtered stream reader would be
...
public class XMLStreamFilterImpl implements javax.xml.stream.XMLStreamReader {
    ...
    /**the very issue around a long discussion. but since we must pass the TCK, we decide to allow 
