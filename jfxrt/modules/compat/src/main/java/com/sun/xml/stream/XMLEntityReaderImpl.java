...
/*
 * $Id: XMLEntityReaderImpl.java,v 1.7 2010-11-02 21:02:26 joehw Exp $
 */
/*
 * XMLEntityReaderImpl.java
 *
 * Created on December 2, 2002, 3:06 PM
 */
...
public class XMLEntityReaderImpl extends XMLEntityReader  {
    ...
    //fields
    //kvr : Below fields should be made non static.
    ...
    //Fix me: Handle case where spaces in literal are more than 100.
    //a rare case.
    int whiteSpaceLen=0;
        ...
        if (XMLChar.isNameStart(fCurrentEntity.ch[offset])) {
            if (++fCurrentEntity.position == fCurrentEntity.count) {
                ...
                if (load(1, false)) {
                    ...
                    scannedName = fSymbolTable.getCharArray();
                    if (DEBUG_BUFFER) {
                        System.out.print(")scanName: ");
                        print();
                        System.out.println(" -> "+String.valueOf(symbol));
        ...
        if (XMLChar.isNameStart(fCurrentEntity.ch[offset])) {
            if (++fCurrentEntity.position == fCurrentEntity.count) {
                ...
                if (load(1, false)) {
                    ...
                    qname.characters = fSymbolTable.getCharArray();
                    if (DEBUG_BUFFER) {
                        System.out.print(")scanQName, "+qname+": ");
                        print();
            ...
            if(whiteSpaceInfoNeeded){
                if(c== 0x20 || c == 0x9){
                    if(whiteSpaceLen < whiteSpaceLookup.length){
                        ...
                        int [] tmp = new int[whiteSpaceLookup.length+20];
    ...
    throws IOException {
        ...
        do {
            ...
            if (fCurrentEntity.position == fCurrentEntity.count) {
                ...
                invokeListeners(fCurrentEntity.count - fCurrentEntity.position);
