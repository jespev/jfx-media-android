...
/*
 * $Id: XMLDocumentFragmentScannerImpl.java,v 1.11 2010-11-02 21:02:26 joehw Exp $
 */
...
import org.apache.xerces.util.XMLAttributesIteratorImpl;
 ...
 * @version $Id: XMLDocumentFragmentScannerImpl.java,v 1.11 2010-11-02 21:02:26 joehw Exp $
...
implements XMLDocumentScanner, XMLComponent, XMLEntityHandler {
    ...
    public void reset(PropertyManager propertyManager){
        ...
        //fSupportExternalEntities = true;
       	Boolean bo = (Boolean)propertyManager.getProperty(ZephyrParserFactory.IS_REPLACING_ENTITY_REFERENCES);
		fReplaceEntityReferences = bo.booleanValue();
       	bo = (Boolean)propertyManager.getProperty(ZephyrParserFactory.IS_SUPPORTING_EXTERNAL_ENTITIES);
    ...
    protected void scanComment() throws IOException, XNIException {
        ...
        // call handler
        if (fDocumentHandler != null) {
            fDocumentHandler.comment(fContentBuffer, null);
        }
...
//        calle should make sure that it doesn't call for value outside allowed co-ordinates        
    ...
    boolean skipQElement(QName name) throws IOException{        
        ...
        final int c = fEntityScanner.getChar(name.characters.length);
        //if this character is still valid element name -- this means string can't match        
        if(XMLChar.isName(c)){
            return false;
        }else{
            return fEntityScanner.skipString(name.characters);
        }        
    }
    ...
    throws IOException, XNIException {
        ...
        //Try the skipping
        //Note that fAdd is only set to false, after analyzing data which
        //has been parsed
        ...
        if(fShouldSkip && !fAdd){
            ...
            //token in the buffer
            QName name = fElementStack2.getNext();
            ...
            fShouldSkip = skipQElement(name);
            
            if(DEBUG_SKIP_ALGORITHM){
                System.out.println("STRING = " + name.rawname + " SKIPPED = " + fShouldSkip);
            }
            /**
            //this means that skipping was sucessful -- 
            if(fShouldSkip){
                //just change the reference
                fElementQName = name;
                fElementStack.setElement(fElementQName);
            }
            else{
                // name
                if (fNamespaces) {
                    ...
                    String element = fEntityScanner.scanName();
                    fElementQName.setValues(null, element, element, null);
        ...
        //Should an element be stored in stack2 -- return the reference 
        //and set the same reference in stack1 too
        if(fAdd){
            fElementQName = fElementStack2.nextElement();
            //fElementStack.setElement(fElementQName);
        }else{
        ...
        //if at all skipping should be done --
        //1. We are still at the stage of adding 
        //2. Last skipping operation failed and skipping has been stopped
        if(!fShouldSkip || fAdd){
            // name
            if (fNamespaces) {
                fEntityScanner.scanQName(fElementQName);
        ...
        if(fAdd){
            fElementStack2.matchElement(fElementQName);
        }
        ...
        // this code will read all the attributes Name, Value pair.
        // It will also consume '>' for start element or '/>' if its empty element.
        do {
            // spaces
        ...
        if (empty) {
            ...
            //Also pass the information if the document handler is registered.
            //Infact this can be layered on top of pull model.
            if(fDocumentHandler != null){
    ...
    protected int scanEndElement() throws IOException, XNIException {
        ...
        String rawname = fElementStack.popElement().rawname ;
...
//            fDocumentHandler.endElement(fElementQName, null);
    ...
    protected void scanEntityReference(XMLStringBuffer content) throws IOException, XNIException {
        ...
        // set the state to ENTITY_REFERENCE & return
        
		if((fEntityStore.isExternalEntity(name) && !fSupportExternalEntities) 
        || (!fEntityStore.isExternalEntity(name) && !fReplaceEntityReferences) ){
            ...
            if(fIsCoalesce){
                ...
                switch(fScannerState){
                    ...
                    case SCANNER_STATE_CHARACTER_DATA: {
                        ...
                        //we should not clear the buffer only when last state was REFERENCE or CDATA or CHARACTER_DATA
