...
#define GLASS_LOG(level,...) \
        if (level == GLASS_LOG_LEVEL_SEVERE) LOGE(TAG, __VA_ARGS__);      \
		else if (level == GLASS_LOG_LEVEL_WARNING) LOGW(TAG, __VA_ARGS__); \
		else if (level == GLASS_LOG_LEVEL_INFO) LOGI(TAG, __VA_ARGS__);    \
		else if (level == GLASS_LOG_LEVEL_CONFIG) LOGE(TAG, __VA_ARGS__);  \
		else if (level == GLASS_LOG_LEVEL_FINE) LOGI(TAG, __VA_ARGS__);    \
		else if (level == GLASS_LOG_LEVEL_FINER) LOGD(TAG, __VA_ARGS__);   \
		else LOGV(TAG, __VA_ARGS__);
#else
#define GLASS_LOG(level,...) \
    GLASS_IF_LOG(level) \
