...
static jobject createJavaScreen(JNIEnv *env, NativeScreen screen) {
    ...
    jobject newScreen = screen != 0 ? (jobject)(*env)->NewObject(env, jScreenClass, screenInit,
        ptr_to_jlong(screen),
        ...
        1.0f):
        (jobject)(*env)->NewObject(env, jScreenClass, screenInit,
...
jobjectArray createJavaScreens(JNIEnv *env) {
    ...
    jobject defScreen = createJavaScreen(env, glass_screen_updateMainScreen(env));
    if (defScreen == NULL) {
