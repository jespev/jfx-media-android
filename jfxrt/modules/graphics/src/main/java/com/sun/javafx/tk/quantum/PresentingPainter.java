...
final class PresentingPainter extends ViewPainter {
    ...
    static boolean afterScreenLost = false;
    ...
    @Override public void run() {
        ...
        try {
            ...
            if (!valid) {
                if (sceneState.getNativeScreen() == 0) {
                    if (!afterScreenLost) {
                        afterScreenLost = true;
                    }                    
            ...
            if (presentable != null && presentable.lockResources(sceneState) ||
                presentable != null && afterScreenLost) {
                disposePresentable();
            }
            if (presentable == null || afterScreenLost) {
                presentable = factory.createPresentable(sceneState);
                penWidth  = viewWidth;
                penHeight = viewHeight;
                if (afterScreenLost) afterScreenLost = false;
            }
