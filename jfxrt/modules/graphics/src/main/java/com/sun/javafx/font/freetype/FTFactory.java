...
public class FTFactory extends PrismFontFactory {
    ...
    public GlyphLayout createGlyphLayout() {
        if (OSFreetype.isPangoEnabled()) {
            try {
                Class clazz = Class.forName("com.sun.javafx.font.freetype.PangoGlyphLayout");
                return (GlyphLayout)clazz.newInstance();
            } catch(Exception ex) {
                System.err.println("Can't load com.sun.javafx.font.freetype.PangoGlyphLayout");
            }
        }
