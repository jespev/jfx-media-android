...
#include <math.h>
#include "SSEUtils.h"
#include "com_sun_scenario_effect_impl_sw_sse_SSEBlend_SOFT_LIGHTPeer.h"

JNIEXPORT void JNICALL
Java_com_sun_scenario_effect_impl_sw_sse_SSEBlend_1SOFT_1LIGHTPeer_filter
...
jint src1w, jint src1h, jint src1scan)
{
    ...
    for (int dy = dsty; dy < dsty+dsth; dy++) {
        ...
        for (int dx = dstx; dx < dstx+dstw; dx++) {
            ...
            float blend_soft_light_res_x, blend_soft_light_res_y, blend_soft_light_res_z, blend_soft_light_res_w;
            {
            ...
            float bot_np_x = bot_tmp_x / bot_tmp_w;
            float bot_np_y = bot_tmp_y / bot_tmp_w;
            float bot_np_z = bot_tmp_z / bot_tmp_w;
            float top_np_x = top_tmp_x / top_tmp_w;
            float top_np_y = top_tmp_y / top_tmp_w;
            float top_np_z = top_tmp_z / top_tmp_w;
            float sqrt_res;
            {
            float x_tmp = bot_np_x;
            sqrt_res = sqrt(x_tmp);

            }
            float sqrtf = sqrt_res;
            float dofx_res;
            {
            float f_tmp = bot_np_x;
            float sqrtf_tmp = sqrtf;
            {
            float d;
            if (f_tmp <= 0.25f){
            d = ((16.0f * f_tmp - 12.0f) * f_tmp + 4.0f) * f_tmp;
            }
             else {
            d = sqrtf_tmp;
            }
            ...
            float dr = dofx_res;
            {
            float x_tmp = bot_np_y;
            sqrt_res = sqrt(x_tmp);

            }
            sqrtf = sqrt_res;
            {
            float f_tmp = bot_np_y;
            float sqrtf_tmp = sqrtf;
            {
            float d;
            if (f_tmp <= 0.25f){
            ...
            float dg = dofx_res;
            {
            float x_tmp = bot_np_z;
            sqrt_res = sqrt(x_tmp);

            }
            sqrtf = sqrt_res;
            {
            float f_tmp = bot_np_z;
            float sqrtf_tmp = sqrtf;
            {
            float d;
            if (f_tmp <= 0.25f){
            ...
            res_x = top_tmp_x;
             ...
             else if (top_tmp_w == 0.0f){
            res_x = bot_tmp_x;
            }
             else if (top_np_x <= 0.5f){
            res_x = bot_tmp_x + (1.0f - bot_tmp_w) * top_tmp_x - top_tmp_w * bot_tmp_x * (1.0f - 2.0f * top_np_x) * (1.0f - bot_np_x);
            }
             else {
            res_x = bot_tmp_x + (1.0f - bot_tmp_w) * top_tmp_x + (2.0f * top_tmp_x - top_tmp_w) * (bot_tmp_w * dr - bot_tmp_x);
            }
            if (bot_tmp_w == 0.0f){
            res_y = top_tmp_y;
            }
            ...
            res_y = bot_tmp_y + (1.0f - bot_tmp_w) * top_tmp_y - top_tmp_w * bot_tmp_y * (1.0f - 2.0f * top_np_y) * (1.0f - bot_np_y);
            }
             else {
            res_y = bot_tmp_y + (1.0f - bot_tmp_w) * top_tmp_y + (2.0f * top_tmp_y - top_tmp_w) * (bot_tmp_w * dg - bot_tmp_y);
            }
            if (bot_tmp_w == 0.0f){
            res_z = top_tmp_z;
            }
            ...
            res_z = bot_tmp_z + (1.0f - bot_tmp_w) * top_tmp_z - top_tmp_w * bot_tmp_z * (1.0f - 2.0f * top_np_z) * (1.0f - bot_np_z);
            }
             else {
            res_z = bot_tmp_z + (1.0f - bot_tmp_w) * top_tmp_z + (2.0f * top_tmp_z - top_tmp_w) * (bot_tmp_w * db - bot_tmp_z);
            }
            blend_soft_light_res_x = res_x;
            blend_soft_light_res_y = res_y;
            blend_soft_light_res_z = res_z;
            blend_soft_light_res_w = res_w;
            }

            }
            color_x = blend_soft_light_res_x;
            color_y = blend_soft_light_res_y;
            color_z = blend_soft_light_res_z;
            color_w = blend_soft_light_res_w;
            }
