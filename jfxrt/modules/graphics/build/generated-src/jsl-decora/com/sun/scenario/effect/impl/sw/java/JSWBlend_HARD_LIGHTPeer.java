...
public class JSWBlend_HARD_LIGHTPeer extends JSWEffectPeer  {
    ...
    public JSWBlend_HARD_LIGHTPeer(FilterContext fctx, Renderer r, String uniqueName) {
                            ...
                            ImageData... inputs)
    {
        ...
        for (int dy = dsty; dy < dsty+dsth; dy++) {
            ...
            for (int dx = dstx; dx < dstx+dstw; dx++) {
                ...
                float blend_hard_light_res_x, blend_hard_light_res_y, blend_hard_light_res_z, blend_hard_light_res_w;
                {
                ...
                float halftopa = 0.5f * top_tmp_w;
                if (top_tmp_x > halftopa){
                res_x = top_tmp_x + bot_tmp_w * (top_tmp_x - top_tmp_w) - bot_tmp_x * (2.0f * top_tmp_x - top_tmp_w - 1.0f);
                }
                 else {
                res_x = 2.0f * bot_tmp_x * top_tmp_x + bot_tmp_x * (1.0f - top_tmp_w) + top_tmp_x * (1.0f - bot_tmp_w);
                }
                if (top_tmp_y > halftopa){
                res_y = top_tmp_y + bot_tmp_w * (top_tmp_y - top_tmp_w) - bot_tmp_y * (2.0f * top_tmp_y - top_tmp_w - 1.0f);
                }
                 else {
                res_y = 2.0f * bot_tmp_y * top_tmp_y + bot_tmp_y * (1.0f - top_tmp_w) + top_tmp_y * (1.0f - bot_tmp_w);
                }
                if (top_tmp_z > halftopa){
                res_z = top_tmp_z + bot_tmp_w * (top_tmp_z - top_tmp_w) - bot_tmp_z * (2.0f * top_tmp_z - top_tmp_w - 1.0f);
                }
                 else {
                res_z = 2.0f * bot_tmp_z * top_tmp_z + bot_tmp_z * (1.0f - top_tmp_w) + top_tmp_z * (1.0f - bot_tmp_w);
                }
                blend_hard_light_res_x = res_x;
                blend_hard_light_res_y = res_y;
                blend_hard_light_res_z = res_z;
                blend_hard_light_res_w = res_w;
                }

                }
                color_x = blend_hard_light_res_x;
                color_y = blend_hard_light_res_y;
                color_z = blend_hard_light_res_z;
                color_w = blend_hard_light_res_w;
                }
