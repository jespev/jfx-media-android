...
#include <math.h>
#include "SSEUtils.h"
#include "com_sun_scenario_effect_impl_sw_sse_SSEBlend_DIFFERENCEPeer.h"

JNIEXPORT void JNICALL
Java_com_sun_scenario_effect_impl_sw_sse_SSEBlend_1DIFFERENCEPeer_filter
...
jint src1w, jint src1h, jint src1scan)
{
    ...
    for (int dy = dsty; dy < dsty+dsth; dy++) {
        ...
        for (int dx = dstx; dx < dstx+dstw; dx++) {
            ...
            float blend_difference_res_x, blend_difference_res_y, blend_difference_res_z, blend_difference_res_w;
            {
            ...
            float min_res_x, min_res_y, min_res_z;
            {
            float x_tmp_x = top_tmp_w * bot_tmp_x;
            float x_tmp_y = top_tmp_w * bot_tmp_y;
            float x_tmp_z = top_tmp_w * bot_tmp_z;
            float y_tmp_x = bot_tmp_w * top_tmp_x;
            float y_tmp_y = bot_tmp_w * top_tmp_y;
            float y_tmp_z = bot_tmp_w * top_tmp_z;
            min_res_x = ((x_tmp_x < y_tmp_x) ? x_tmp_x : y_tmp_x);
            min_res_y = ((x_tmp_y < y_tmp_y) ? x_tmp_y : y_tmp_y);
            min_res_z = ((x_tmp_z < y_tmp_z) ? x_tmp_z : y_tmp_z);

            }
            res_x = bot_tmp_x + top_tmp_x - 2.0f * min_res_x;
            res_y = bot_tmp_y + top_tmp_y - 2.0f * min_res_y;
            res_z = bot_tmp_z + top_tmp_z - 2.0f * min_res_z;
            blend_difference_res_x = res_x;
            blend_difference_res_y = res_y;
            blend_difference_res_z = res_z;
            blend_difference_res_w = res_w;
            }

            }
            color_x = blend_difference_res_x;
            color_y = blend_difference_res_y;
            color_z = blend_difference_res_z;
            color_w = blend_difference_res_w;
            }
