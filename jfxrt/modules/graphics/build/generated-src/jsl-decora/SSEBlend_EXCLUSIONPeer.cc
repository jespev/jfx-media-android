...
#include <math.h>
#include "SSEUtils.h"
#include "com_sun_scenario_effect_impl_sw_sse_SSEBlend_EXCLUSIONPeer.h"

JNIEXPORT void JNICALL
Java_com_sun_scenario_effect_impl_sw_sse_SSEBlend_1EXCLUSIONPeer_filter
...
jint src1w, jint src1h, jint src1scan)
{
    ...
    for (int dy = dsty; dy < dsty+dsth; dy++) {
        ...
        for (int dx = dstx; dx < dstx+dstw; dx++) {
            ...
            float blend_exclusion_res_x, blend_exclusion_res_y, blend_exclusion_res_z, blend_exclusion_res_w;
            {
            ...
            res_x = top_tmp_x + bot_tmp_x - (2.0f * top_tmp_x * bot_tmp_x);
            res_y = top_tmp_y + bot_tmp_y - (2.0f * top_tmp_y * bot_tmp_y);
            res_z = top_tmp_z + bot_tmp_z - (2.0f * top_tmp_z * bot_tmp_z);
            blend_exclusion_res_x = res_x;
            blend_exclusion_res_y = res_y;
            blend_exclusion_res_z = res_z;
            blend_exclusion_res_w = res_w;
            }

            }
            color_x = blend_exclusion_res_x;
            color_y = blend_exclusion_res_y;
            color_z = blend_exclusion_res_z;
            color_w = blend_exclusion_res_w;
            }
