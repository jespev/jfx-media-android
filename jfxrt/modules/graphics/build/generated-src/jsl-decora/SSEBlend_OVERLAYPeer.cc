...
#include <math.h>
#include "SSEUtils.h"
#include "com_sun_scenario_effect_impl_sw_sse_SSEBlend_OVERLAYPeer.h"
...
jint src1w, jint src1h, jint src1scan)
{
    ...
    for (int dy = dsty; dy < dsty+dsth; dy++) {
        ...
        for (int dx = dstx; dx < dstx+dstw; dx++) {
            ...
            ceil_res_x = ceil(x_tmp_x);
            ceil_res_y = ceil(x_tmp_y);
            ceil_res_z = ceil(x_tmp_z);
            ...
            abs_res_x = fabs(x_tmp_x);
            abs_res_y = fabs(x_tmp_y);
            abs_res_z = fabs(x_tmp_z);
